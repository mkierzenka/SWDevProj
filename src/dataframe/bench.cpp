...
int main()
{
  ...
  parseDataFile(df, dataFilename, 1100000);
  validateDF(df); // Crash if data isn't read in correctly

  std::cout << "Benchmarking...\n";
  // Part 1A -- LengthRower test, singlethreaded
  ...
  // Part 1B -- LengthRower test, multithreaded
  ...
  // Part 2 -- FindRower test, singlethreaded
  ...
  // Part 2 -- FindRower test, multithreaded
  startTime = std::chrono::high_resolution_clock::now();
  df->pmap(*findRower);
  endTime = std::chrono::high_resolution_clock::now();
  duration = std::chrono::duration_cast<std::chrono::microseconds>(endTime - startTime);
